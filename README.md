...
한국어는 기본 주소(`/`), 영어는 `/en/`에서 제공한다. 현재 한영 전환 대상은 Loop 엔지니어링 글이며 기존 글은 순차적으로 번역한다.

- 같은 문서는 `content/posts/tech/YYYY-MM-DD-title.ko.md`와 `.en.md`로 작성하고 같은 `translationKey`를 사용한다. 발행일과 기술적 의미를 맞추되 제목·설명·본문·대체 텍스트는 해당 언어로 작성한다.
- 상단 언어 선택은 같은 문서의 번역판으로 이동한다. 번역이 없으면 이동 링크 대신 번역 없음 표시가 나온다. 홈페이지에서는 언어별 홈페이지로 이동한다.
- 글씨가 포함된 그림은 언어별 파일을 연결하고 사진·공통 자산은 공유한다. 영어 글의 내부 링크가 의도치 않게 한국어 문서로 이동하지 않는지 확인한다.
- 로컬 미리보기는 `hugo server --destination /tmp/nsoft-blog-preview`처럼 별도 출력 폴더를 사용한다. 저장소가 추적하는 `public/`에 미리보기 파일을 덮어쓰지 않는다.
- 게시 전 두 언어의 상단 버튼 왕복 이동, 모바일 배치, 기존 주소, canonical·hreflang과 언어별 이미지 경로를 확인한다.
